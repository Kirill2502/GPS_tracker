...
class SettingFragment: PreferenceFragmentCompat() {
    ...
    private fun onChangeListener(): Preference.OnPreferenceChangeListener {
        return Preference.OnPreferenceChangeListener { pref, value ->
            when(pref){
                timePref -> onTimeChange(value.toString())
                colorPref -> pref.icon?.setTint(Color.parseColor(value.toString()))
            }



            true // Явный возврат true из лямбды
        }
    }
    ...
    private fun initPrefs(){
        ...
        timePref.title = "$title: ${nameArray[pos]} "
        val trackColor = pref?.getString("color_key","#14347C")
